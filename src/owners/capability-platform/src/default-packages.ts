import type { CapabilityPackageRecord } from "./records.js";

export const DEFAULT_PACKAGES: CapabilityPackageRecord[] = [
  {
    packageId: "cap.text.normalizer",
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
    lifecycleState: "draft",
    draft: {
      name: "Text Normalizer",
      description: "Normalize inbound text before downstream review or package consumption.",
      metadata: {
    ...
    gates: ["draft-edit", "publish", "history-read", "executor-boundary-required"],
  ...
  {
    packageId: "cap.change.summary",
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
    lifecycleState: "draft",
    draft: {
      name: "Change Summary",
      description: "Produce a bounded textual summary of changed files for downstream audit or review surfaces.",
      metadata: {
        owner: "capability-platform",
        audience: "internal"
      },
      entrypoint: "summarize_changes",
      sourceLanguage: "python",
      packageKind: "function",
      sourceMode: "system-bootstrap",
      componentDependencies: [],
      ...
      sourceText: [
        ...
        "    return f\"Changed files: {', '.join(sorted(paths))}\""
      ].join("\n")
    },
    gates: ["draft-edit", "publish", "rollback", "executor-boundary-required"],
    surfaceContracts: ["unpublished-draft", "published-source", "published-text", "rollback"],
    releaseIds: [],
    publicationIds: [],
    eventIds: [],
    policyIds: []
  ...
  {
    packageId: "cap.workflow.build-repo-tree-and-gitignore",
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
    lifecycleState: "draft",
    draft: {
      name: "Build Repo Tree And Gitignore",
      description: "Build the repo tree file and generated gitignore output for the Skeleton repository.",
      metadata: {
        owner: "capability-platform",
        audience: "internal"
      },
      entrypoint: "build_repo_tree_and_gitignore",
      sourceLanguage: "python",
      packageKind: "function",
      sourceMode: "system-bootstrap",
      componentDependencies: [],
      ...
      sourceText: [
        ...
        "",
        "OUTPUT_ROOT = Path(os.environ.get(\"SKELETON_OUTPUT_ROOT\", \"./outputs\")).resolve()",
        "",
        "",
        "@dataclass",
        "class FileNode:",
        ...
        "",
        "",
        "def make_repo_tree_filename(target_repo: Path) -> Path:",
        "    safe_name = target_repo.name.strip() or \"repo\"",
        "    return OUTPUT_ROOT / f\"{safe_name} repo tree.txt\"",
        "",
        "",
        "def make_gitignore_output_filename(target_repo: Path) -> Path:",
        "    safe_name = target_repo.name.strip() or \"repo\"",
        "    return OUTPUT_ROOT / f\"{safe_name} generated gitignore.txt\"",
        "",
        "",
        ...
        "",
        "",
        "def make_repo_tree_diff_filename(target_repo: Path) -> Path:",
        "    safe_name = target_repo.name.strip() or \"repo\"",
        "    return OUTPUT_ROOT / f\"{safe_name} repo tree diff.txt\"",
        "",
        "",
        "def make_gitignore_diff_filename(target_repo: Path) -> Path:",
        "    safe_name = target_repo.name.strip() or \"repo\"",
        "    return OUTPUT_ROOT / f\"{safe_name} generated gitignore diff.txt\"",
        "",
        "",
        ...
        "",
        "",
        "def write_gitignore_with_diff(workflow: TreeVerificationWorkflow, target_repo: Path) -> None:",
        "    output_file = workflow.output_file",
        "    diff_file = make_gitignore_diff_filename(target_repo)",
        "    temp_file = make_temp_filename(output_file)",
        "",
        ...
        "",
        "",
        "def build_repo_tree_and_gitignore(mark_safe: str | None = None) -> dict[str, str]:",
        "    target_repo = Path(os.environ.get(\"SKELETON_TARGET_REPO\", \".\")).resolve()",
        ...
        "",
        "    tree_file = make_repo_tree_filename(target_repo)",
        "    tree_diff_file = make_repo_tree_diff_filename(target_repo)",
        "    gitignore_output_file = make_gitignore_output_filename(target_repo)",
        "    gitignore_diff_file = make_gitignore_diff_filename(target_repo)",
        "",
        "    build_repo_tree(",
        ...
        "        workflow.run()",
        "",
        "    write_gitignore_with_diff(workflow, target_repo)",
        "",
        "    return {",
